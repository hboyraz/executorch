...
class RemovePermutesAroundElementwiseOps(ExportPass):
    ...
    class Subgraph:
        ...
        node_start_permute: dict[torch.fx.Node, list[int]] = field(
            default_factory=dict
        )

    # Ops explicitly listed as permutable. Includes non-pointwise ops
    ...
    # In addition, view_copy ops that insert/remove a single size-1
    # dimension are accepted as rank-changing boundaries
    # (see _is_squeeze_unsqueeze_view).
    _base_permutable_ops: set = set()
    _base_ops_initialized = False

    def __init__(self, extra_permutable_ops: set | None = None) -> None:
        super().__init__()
        self._extra_permutable_ops = extra_permutable_ops or set()

    @classmethod
    def _get_base_permutable_ops(cls) -> set:
        if not cls._base_ops_initialized:
            cls._base_permutable_ops = {
            ...
            try:
                ...
                pass
            cls._base_ops_initialized = True
        return cls._base_permutable_ops

    def _is_in_permutable_ops(self, target) -> bool:
        return target in self._get_base_permutable_ops() or target in self._extra_permutable_ops
    ...
    def _is_squeeze_unsqueeze_view(self, node: torch.fx.Node) -> bool:
        ...
        if not isinstance(inp, torch.fx.Node) or inp.meta.get("val") is None:
            return False
        in_shape = list(inp.meta["val"].shape)
    ...
    def _adapt_permute_across_view(
        ...
        if node.target in self._SQUEEZE_OPS:
            ...
            # Find where the squeezed input dim appears in the permutation
            if index not in permute:
                return None
            new_perm = [x - 1 if x > index else x for x in permute if x != index]
            return new_perm
        ...
        if len(out_shape) == len(in_shape) + 1:
            ...
            if index == -1 or index not in permute:
                return None
            new_perm = [x - 1 if x > index else x for x in permute if x != index]
            return new_perm
        return None
            ...
            if (
                ...
                if adapted_start is not None:
                    ...
                    for view_user in view_node.users:
                        ...
                        if self.visit(
                            view_user, subgraph, processed_nodes,
                            adapted_end, adapted_start
                        ):
                            subgraphs_found.append(subgraph)
                            for n in subgraph.nodes:
                                processed_nodes.add(n)
    ...
    def visit(  # noqa: C901
        ...
        if self._is_squeeze_unsqueeze_view(node):
            # Adapt end permute for downstream (input-rank → output-rank)
            adapted_end = self._adapt_permute_across_view(
                current_end_permute, node
            )
            if adapted_end is None:
                return False
            downstream_end = adapted_end

            # Adapt start permute for downstream too (so upstream checks
            # on the other side of the view use the correct rank)
            adapted_start = self._adapt_permute_across_view(
                current_start_permute, node
            )
        ...
        for user in node.users:
            if user.target == exir_ops.edge.aten.permute_copy.default:
                ...
                if user_perm == downstream_end:
                    ...
                    if (
                        len(user_users) == 1
                        and self._is_squeeze_unsqueeze_view(user_users[0])
                    ):
                        view_after = user_users[0]
                        # Adapt the expected end permute across the view
                        adapted = self._adapt_permute_across_view(
                            downstream_end, view_after
                        )
                        if adapted is not None and user_perm == adapted:
        ...
        for inp in node.all_input_nodes:
            if inp.target == exir_ops.edge.aten.permute_copy.default:
                ...
                inp, subgraph, processed_nodes,
                current_end_permute, current_start_permute
            ):
                return False
    ...
    def is_node_permutable(self, node: torch.fx.Node) -> bool:
        if self._is_in_permutable_ops(node.target):
            if node.target in (
                exir_ops.edge.aten.mean.dim,
                exir_ops.edge.aten.sum.dim_IntList,
            ):
        ...
        if self._is_squeeze_unsqueeze_view(node):
            return True
        return self._is_in_permutable_ops(node.target)
    ...
    def permute_subgraph(self, subgraph: Subgraph) -> None:
        ...
        for node in subgraph.nodes:
            ...
            elif node.target in self._UNSQUEEZE_OPS or node.target in self._SQUEEZE_OPS:
                dim = cast(int, node.args[1])
